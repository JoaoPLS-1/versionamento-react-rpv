...
export function InputMask({
    ...
    return(
        <div className={`col-span-12 sm:col-span-4 ${sizeClassMap[size]} flex flex-col `}>
            <label>{label}{required && (<span className='text-red-500 mb-4 mt-0'>*</span>)}: </label>
            <input {...registration} className='border rounded-md px-2 py-1 text-zinc-100 bg-black' placeholder={placeholder} maxLength={maxLength} onChange={handleChange} />
            <span className='top-16 text-xs text-red-500'>{errors[name]?.message}</span>
        </div>
    )
}